// Single source of truth for motion values (ECC motion-foundations).
// Components must pull durations, easings and springs from here.

type Bezier = [number, number, number, number]

export const motionTokens = {
  duration: {
    instant: 0.08,
    fast: 0.18,
    normal: 0.35,
    slow: 0.6,
    crawl: 1.0,
    drift: 2.8,
  },
  easing: {
    smooth: [0.22, 1, 0.36, 1] as Bezier,
    sharp: [0.4, 0, 0.2, 1] as Bezier,
    bounce: [0.34, 1.56, 0.64, 1] as Bezier,
    linear: [0, 0, 1, 1] as Bezier,
  },
  distance: {
    xs: 4,
    sm: 8,
    md: 16,
    lg: 24,
    xl: 48,
  },
  scale: {
    subtle: 0.98,
    press: 0.95,
    pop: 1.04,
  },
}

export const springs = {
  snappy: { type: "spring" as const, stiffness: 300, damping: 30 },
  gentle: { type: "spring" as const, stiffness: 120, damping: 14 },
  bouncy: { type: "spring" as const, stiffness: 400, damping: 10 },
  instant: { type: "spring" as const, stiffness: 600, damping: 35 },
  release: { type: "spring" as const, stiffness: 200, damping: 20, restDelta: 0.001 },
}

// Imperative (rAF) timings that aren't driven by motion/react.
export const demoTimings = {
  sessionMs: 20_000,
  loadingLineMs: 2_600,
  wheelSpinMs: 5_000,
  wheelBigSpinMs: 7_200,
  wheelReducedMs: 900,
  cutToWheelMs: 3_000,
  // How long you can be away (tab hidden / bar off-screen) before the session forfeits.
  leaveGraceMs: 1_500,
}
